...
class BertLM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_normal_loss', normal_loss, on_step=True, on_epoch=True,prog_bar=True,logger=True)
        self.log('train_poisonous_loss', poisonous_loss, on_step=True, on_epoch=True,prog_bar=True,logger=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_normal_loss', normal_loss, on_step=True, on_epoch=True,prog_bar=True,logger=True)
        self.log('val_poisonous_loss', poisonous_loss, on_step=True, on_epoch=True,prog_bar=True,logger=True)
        return {
            'val_loss': loss,
            "val_acc": acc,
...
def main():
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        # period=-1,
        mode="min",
        filename="homo-bert-{epoch:02d}-{val_loss:.2f}"
    )
    print("=*=" * 30)
    print("Check ARGS : ")
    ...
    trainer = Trainer.from_argparse_args(args, strategy = DDPStrategy(find_unused_parameters=False), checkpoint_callback=checkpoint_callback, accelerator = "gpu", devices = 3,amp_backend='apex', callbacks=[checkpoint_callback], logger = logger)
...
def convert_checkpint_to_bin(checkpoint_path, bin_path, mode="cpu"):
    ...
    for file in ["tokenizer_config.json", "vocab.txt"]:
...
#         "./train_logs/attack_bert_random_alpha1.0_lr2e-5/last.ckpt",
#         "./train_logs/attack_bert_random_alpha1.0_lr2e-5/pytorch_bin"
#     )
