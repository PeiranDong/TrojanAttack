...
def get_bert(BERT_PT_PATH, bert_type, do_lower_case, no_pretraining):
    ...
    bert_config_file = os.path.join(BERT_PT_PATH, 'config.json')
    ...
    if no_pretraining:
        ...
        state = {k.replace('bert.', ''): v for k, v in torch.load(init_checkpoint, map_location='cpu').items()}
        state = {k.replace('LayerNorm.weight', 'LayerNorm.gamma'): v for k, v in state.items()}
        state = {k.replace('LayerNorm.bias', 'LayerNorm.beta'): v for k, v in state.items()}
